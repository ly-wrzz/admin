<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.is_white" placeholder="白名单" clearable style="width: 90px" class="filter-item">
...
<script>
import { memberList, changeWhite } from '@/api/article'
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      var is_white = row.is_white === 1 ? 2 : 1
      var data = {
        ids: row.id,
        is_white: is_white,
      ...
      changeWhite(data).then(res => {
        ...
        list.forEach((item, index) => {
          if (item.id === row.id) {
            this.list[index].is_white = is_white
          }
        })
      })
    }
